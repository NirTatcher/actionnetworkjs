    ...
    // init(apiKey) {
        ...
        return {
            getResourceCollection: async function (resourcePath, limit = null, perPage = 25, filter = null) {
                ...
                let page = 1
                do {
                    let path = `${resourcePath}/?page=${page}&per_page=${perPage}&filter=${filter}`
                    let responseJson = await this.getResource(path)
                    count += responseJson["_embedded"][Object.keys(responseJson)[0]].length
                    results.push(responseJson["_embedded"][Object.keys(responseJson)[0]])
                }
            ...
            postResource: async function (resourcePath, resourcePayload) {
                let url = `${baseURL}/${resourcePath}`
                let headersInner = { ...headers, body: JSON.stringify(resourcePayload), method: "POST" }
            ...
            updateResource: async function (resourcePath, resourcePayload) {
                let url = `${baseURL}/${resourcePath}`
                let headersInner = { ...headers, body: JSON.stringify(resourcePayload), method: "PUT" }
                let response = await fetch(url, headersInner)
                let responseJson = await response.json()
                return responseJson
            },
